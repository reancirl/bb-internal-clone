...
class TradePartnerController extends Controller
{
    public function index(Request $request): JsonResponse
    {
        ...
        $partners = TradePartner::query()
            ...
            ->paginate($perPage)
            ->withQueryString()
            ->through(fn (TradePartner $p) => $this->transform($p));
    ...
    public function store(Request $request): JsonResponse
    {
        ...
        return response()->json([
            'message' => 'Trade partner added.',
            'partner' => $this->transform($partner->fresh('trades')),
        ], 201);
    }

    public function update(Request $request, TradePartner $tradePartner): JsonResponse
    {
        $data = $this->validateData($request);
        ...
        return response()->json([
            'message' => 'Trade partner updated.',
            'partner' => $this->transform($tradePartner->fresh('trades')),
        ]);
    }

    public function destroy(TradePartner $tradePartner): JsonResponse
    {
        $tradePartner->delete();

        return response()->json(['message' => 'Trade partner removed.']);
    }

    /**
     * @return array<string, mixed>
     */
    private function transform(TradePartner $p): array
    {
        return [
            'id' => $p->id,
            'name' => $p->name,
